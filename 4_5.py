        ...
        def p_variable(p):
            ...
            '''elif type(p[1]) == list:  # 如果ID是list，则为record
                ...
                if i not in possiable_token:
                    report_error("The variable used is not defined",p.slice[1].lineno,[i], p.slice[1].lexpos + p.slice[1].lineno - 1, p.slice[1].lexpos + p.slice[1].lineno - 1 + len(p[1]))
                # 错误类型：使用的变量未定义
                ...
                for j in p[1][1:]:  # record内部项
                    ...
                    if j not in possiable_token:
                        report_error("The variable used is not defined.",p.slice[1].lineno,[j],p.slice[1].lexpos + p.slice[1].lineno - 1,p.slice[1].lexpos + p.slice[1].lineno - 1 + len(p[1]))
                    # 错误类型：使用的变量未定义
        ...
        def p_expression(p):
            ...
            if len(p) == 4:  # expression : simple_expression RELOP simple_expression
                ...
                '''elif p[1]["__type"] == "RECORD" or p[3]["__type"] == "RECORD":
                    report_error("Compare the RECORD type",p.lexer.lineno,"",p.lexer.lexpos)
                # 错误类型：对RECORD类型进行比较+'''
        ...
        def p_expression_equal(p):
            ...
            '''if p[1]["__type"] == "RECORD" or p[3]["__type"] == "RECORD":
                report_error("Compare the RECORD type",p.lexer.lineno,"",p.lexer.lexpos) # 错误类型：对RECORD类型进行比较'''
        ...
        def p_error(p):
            report_error("ungrammatical",p.lineno if p else 0,[p.value if p else ""], p.lexpos if p else 0)
            # 错误类型：不符合语法
