    ...
    def __init__(self):
            ...
            p[0]["SymbolTable"] = p[1]["SymbolTable"] + [
                {
                    ...
                    # 判断 type
                    "type": "INTEGER"
                    if type(p[5]["value"]) == int
                    else ("REAL" if type(p[5]["value"]) == float else "CHAR"),
                    "value": p[5],
                    # 非 char 类型整数则标记 positive
                    "positive": True
                    if type(p[5]["value"]) != str and p[5]["value"] > 0
                    else False,
                }
            ]
            ...
            p[0]["SymbolTable"] = [
                {
                    ...
                    else ("REAL" if type(p[3]["value"]) == float else "CHAR"),
                    "value": p[3],
                    "positive": True
                    if type(p[3]["value"]) != str and p[3]["value"] > 0
                    else False,
                }
            ]
            self.symbolMap[self.id] = {
        ...
        def p_type(p):
            ...
            if not type(p[1]) == dict and p[1].upper() == "ARRAY":
                ...
                p[0] = {"length": len(p), "type": "type", "_type": p[1]}
                p[0]["SymbolTable"] = {
        ...
        def p_basic_type(p):
            ...
            basic_type : INTEGER
                        ...
                        | CHAR
            """
            p[0] = {"type": "basic_type", "_type": p[1].upper()}
        ...
        def p_parameter(p):
            ...
            parameter : var_parameter
                    | value_parameter
            """
            p[0] = {"length": len(p), "type": "parameter", "value": p[1]}
            # 符号表从产生式右侧获取
            p[0]["SymbolTable"] = p[1]["SymbolTable"]
        ...
        def p_var_parameter(p):
            ...
            """
            p[0] = {"length": len(p), "type": "var_parameter", "value_parameter": p[2]}
            p[0]["SymbolTable"] = {
                # 函数定义变量是引用调用
        ...
        def p_statement(p):
            ...
            # statement : FOR ID ASSIGNOP expression TO expression DO statement
                ...
                if (
                    p[2] not in list(self.curSymbol.keys())
                    and self.inSubFun
                    and p[2] not in list(self.subSymbol.keys())
                ):
                ...
                if id:
                    if p[4]["__type"] == "UNDEFINED":  # expression未定义
                        ...
                        not p[4]["__type"]
                        or id["type"] not in safe_assign[p[4]["__type"]]
                    ):  # 不是安全赋值类型
                        if (
                            p[4]["__type"] and id["type"] in warn_assign[p[4]["__type"]]
                        ):  # 属于warn复制类型
                            if not self.warning:
                                self.warning = []
                            self.warning += [
            ...
            #  statement : variable ASSIGNOP expression
                ...
                if p[3]["__type"] == "UNDEFINED":  # expression未定义
                    ...
                    if (
                        not p[3]["__type"]
                        or id["type"] not in safe_assign[p[3]["__type"]]
                    ):
        ...
        def p_variable_list(p):
            ...
            if len(p) == 4:  # variable_list : variable_list COM variable
                ...
                p[0] = {"length": len(p), "type": "variable_list", "variables": [p[1]]}
        ...
        def p_variable(p):
            ...
            p[0] = {
                ...
                "type": "variable",
                # ID类型
                "__type": self.search_symbol(p[1])["type"]
                if self.search_symbol(p[1])
                else "UNDEFINED",
                "ID": p[1],
                "id_varpart": p[2],
            }
            if (
                type(p[1]) == str
                and p[1] not in list(self.curSymbol.keys())
                and not (self.inSubFun and p[1] in list(self.subSymbol.keys()))
            ):
                # 如果ID是字符串但未定义
                if not self.error:
                    self.error = []
                ...
                if i not in possiable_token:
                    ...
                    possiable_token = [
                        j["token"]
                        for j in self.search_symbol(i)["recordTable"]["variables"]
                    ]  # possiable_token符号表成为该record的变量表
                ...
                for j in p[1][1:]:  # record内部项
                    ...
                    # 该项已定义，在recordTable内部查，逐层循环
                        ...
                        possiable_token = [
                            k["token"]
                            for k in self.search_symbol(j, record_table)["recordTable"][
                                "variables"
                            ]
                        ]
                        record_table = self.search_symbol(j, record_table)[
                            "recordTable"
                        ]
                    else:  # 最终层，确定最终该变量类型
                        p[0]["__type"] = self.search_symbol(j, record_table)["type"]
        ...
        def p_procedure_call(p):
            ...
            if len(p) == 2:  # procedure_call : ID
                p[0] = {"length": len(p), "type": "procedure_call", "ID": p[1]}
                    ...
                    self.error += [
                        {
                            ...
                            "info": {
                                ...
                                "value": [
                                    "0",
                                    len(
                                        self.subFuncMap[p[1]]["variables"]
                                        if self.subFuncMap[p[1]]["variables"]
                                        else []
                                    ),
                                ],
                                "lexpos": p.slice[1].lexpos + p.slice[1].lineno - 1,
                ...
                if len(p[3]["__type"]) != len(
                    ...
                    for i in range(len(p[3]["__type"])):  # 遍历expression_list
                                ...
                                self.warning += [
                                    {
                                        ...
                                        "info": {
                                            "line": p.lexer.lineno,
                                            "value": [
                                                self.subFuncMap[p[1]]["variables"][i][
                                                    "token"
                                                ],
                                                from_type,
                                                to_type,
                        ...
                        if self.subFuncMap[p[1]]["references"][i] and not (
                            p[3]["expressions"]
                            and p[3]["expressions"][i]
                            and p[3]["expressions"][i]["length"] == 2
                            and p[3]["expressions"][i]["simple_expression"]["length"]
                            == 2
                            and p[3]["expressions"][i]["simple_expression"]["term"][
                                "length"
                            ]
                            == 2
                            and p[3]["expressions"][i]["simple_expression"]["term"][
        ...
        def p_else_part(p):
            ...
            if len(p) == 3:  # else_part : ELSE statement
                p[0] = {"length": len(p), "type": "else_part", "statement": p[2]}
            else:  # else_part为空
                p[0] = {"length": len(p), "type": "else_part", "statement": None}
        ...
        def p_expression_list(p):
            ...
            if len(p) == 4:  # expression_list : expression_list COM expression
                p[0] = {
                    ...
                    "expressions": p[1]["expressions"] + [p[3]]
                    if p[3]
                    else p[1]["expressions"],
                }
            ...
            if len(p) == 4:  # expression : simple_expression RELOP simple_expression
                ...
                if p[1]["__type"] == "UNDEFINED" or p[3]["__type"] == "UNDEFINED":
                    ...
                    self.error += [
                        {
                            "code": "C-09",
                            "info": {"line": p.lexer.lineno, "lexpos": p.lexer.lexpos},
                        }
                    ]  # 错误类型：对未定义的变量进行比较
        ...
        def p_term(p):
            ...
            if len(p) == 4:  # term : term MULOP factor
                ...
                if (
                    p[1]["__type"] == "UNDEFINED" or p[3]["__type"] == "UNDEFINED"
                ):  # term或factor中标识符未定义
                    if not self.error:
                        self.error = []
                    self.error += [
                    ...
                    p[1]["__type"] == "RECORD" or p[3]["__type"] == "RECORD"
                ):  # term或factor中标识符为RECORD
                    if not self.error:
                        self.error = []
                    self.error += [
                        {
            ...
            if len(p[3]["__type"]) != len(
                ...
                for i in range(len(p[3]["__type"])):  # 遍历expression_list
                    ...
                    if self.subFuncMap[p[1]]["references"][i] and not (
                        ...
                        and p[3]["expressions"][i]
                        and p[3]["expressions"][i]["length"] == 2
                        and p[3]["expressions"][i]["simple_expression"]["length"] == 2
                        and p[3]["expressions"][i]["simple_expression"]["term"][
                            "length"
                        ]
                        == 2
        ...
        def p_multype(p):
            ...
            if len(p) == 6:  # multype : multype idlist COLON type SEMICOLON
                p[0] = {
                    ...
                    "id": self.id,
                    "type": "multype",
                    "values": p[1]["values"] + [{"idlist": p[2], "type": p[4]}],
                }
                p[0]["SymbolTable"] = {
                    "variables": p[1]["SymbolTable"]["variables"]
                    + [
