...
#     # 构造数据文件夹的路径
#     test_path = os.path.join(current_path, "testcodes")
#     # 遍历文件夹中的所有文件
...
#                 fi.write(res)
#             fi.close()
#         tree = json.loads(res)['ast']
#         if tree==None:
#             continue
#         ofile = open('testResults/{}.c'.format(file_name), 'w')
#         program = ProgramStructNode(tree)
#         program.Parse()
#         Output.FormatOutput(ofile)
#         ofile.close()

ifile = open('testresults/1.pas.json', 'r')
ofile = open('testResults/1.pas.c', 'w')
